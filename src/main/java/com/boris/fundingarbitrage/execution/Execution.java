...
public class Execution {
	...
	private final Map<BaseExchange, BigDecimal> spotBalances = new ConcurrentHashMap<>();
	private final Map<BaseExchange, BigDecimal> futuresBalances = new ConcurrentHashMap<>();
	private final Map<BaseExchange, ExchangeChains> exchangeChains = new ConcurrentHashMap<>();

	private final CompletableFuture<Void> initFuture;

	public Execution(BaseExchange longExchange, BaseExchange shortExchange, BigDecimal amount) {
	...
	private CompletableFuture<Void> fillBalancesMap() {
		...
		for (BaseExchange exchange : Instances.getExchangeArray()) {
			CompletableFuture<Void>
							spotBalanceFuture =
							exchange.privateHttpClient.getSpotUsdtBalance().thenAccept(spotB -> spotBalances.put(exchange, spotB));
			CompletableFuture<Void>
							futuresBalanceFuture =
							exchange.privateHttpClient.getFuturesUsdtBalance()
	...
	private CompletableFuture<Void> fillChainsMap() {
		...
		for (BaseExchange exchange : Instances.getExchangeArray()) {
			CompletableFuture<Void>
							chainsFuture =
							exchange.privateHttpClient.getSupportedChains()
	...
	public CompletableFuture<Void> withdrawToExchanges() {
		...
		OptimalWithdrawer
						withdrawer =
						new OptimalWithdrawer(longExchange, shortExchange, amount, spotBalances, futuresBalances, exchangeChains);

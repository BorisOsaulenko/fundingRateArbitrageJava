...
public class PrettyWsClient {
    ...
    private static final ClientManager client;
    ...
    private void connect() {
        try {
            ...
            Logger.getInstance().error("Deployment exception. Wrong endpoint config: " + e.getMessage());
        } catch (IOException e) {
            Logger.getInstance().error("Network/protocol issue: " + e.getMessage());
        }
    }

    public void sendMessage(String message) {
        if (lastSession != null && lastSession.isOpen()) {
            lastSession.getAsyncRemote().sendText(message, result -> {
                if (!result.isOK()) {
    ...
    public <T> void sendObject(T obj) {
        try {
            ...
            Logger.getInstance().error("Failed to send object as JSON: " + e.getMessage());
        }
    }
    ...
    public void close() {
        ...
        if (lastSession != null && lastSession.isOpen()) {
            try {
                ...
                Logger.getInstance().error("Error closing WebSocket session: " + e.getMessage());
            }
        }
    ...
    private Consumer<Session> getOnCloseHook() {
        return session -> {
            ...
            if (closeRequested) {
                Logger.getInstance().log("WebSocket closed normally as requested.");
                lastSession = null;
                connecting.completeExceptionally(new IllegalStateException("Client closed"));
                return;
            }
